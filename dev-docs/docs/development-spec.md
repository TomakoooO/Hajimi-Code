...
| FE-047 | SettingsView 重构与 SKILL 设置页 | 已完成 | FE-005 | P1 | 1d | 将设置抽屉重构为独立的 Tab 页，支持左侧菜单切换模型设置和 SKILL 列表，支持点击 SKILL 在编辑器预览 | 新增 `SettingsView.vue` 及 `/api/ide/skills` 接口 |
| FE-048 | 增加自定义 SKILL 创建流程及文件上传 | 已完成 | FE-047 | P1 | 1d | 用户输入名称和描述并上传文档，由后端生成标准 SKILL 文件夹及 SKILL.md | |
| FE-049 | 增加全局任务看板（任务查看） | 已完成 | FE-048 | P1 | 1d | 在 SettingsView 增加任务查看页面，读取项目级 `.tasks`，并进行列表与依赖关系的渲染 | |
